...
* **Списки (Seq)** - связные списки однотипных элементов;

* **Отображения (Map)** - данная коллекция позволяет хранить пары ключ-значение, например, `Integer -> String`;

* **Множества (Set)** - уникальные однотипные элементы;

* **Кортежи (Tuple)** - элементы разных типов.
