...
элементы, но также `вставлять` элементы, `удалять` элементы и эффективные дополнения новых элементов в конце буфера. 
Основными новыми методами, поддерживаемыми буфером, являются **`+=`** и **`++=`** для добавления элементов в конец, 
...
Двумя часто используемыми реализациями буферов являются `ListBuffer` и `ArrayBuffer`. Как следует из названия,
...
------------------------- | --------------------
**Добавление**            |
`buf += x`                | Добавляет элемент `x` в конец буфера и возвращает `buf` как результат.
`buf += (x, y, z)`        | Добавляет заданные элементы в конец буфера.
`buf ++= xs`              | Добавляет все элементы `xs` в в конец буфера.
`x +=: buf`               | Добавляет элементы `x` в начало буфера.
`xs ++=: buf`             | Добавляет все элементы `xs` в начало буфера.
`buf insert (i, x)`       | Вставляет элемент `x` по индексу `i` в буфер.
`buf insertAll (i, xs)`   | Вставляет все элементы в `xs` по индексу `i` в буфер.
**Удаление**              |
`buf -= x`                | Удаляет элемент `x` из буфера.
`buf remove i`            | Удаляет элемент по индексу `i` из буфера.
`buf remove (i, n)`       | Удаляет `n` элементов, начиная с индекса `i` из буфера.
`buf trimStart n`         | Удаляет первые `n` элементов из буфера.
